...
VIJIM Labs 的唯一业务 UI 出口。本仓库公开可见，但不包含业务数据、生产配置或密钥。
...
`npm run check` 会依次执行 TypeScript 检查和生产构建。生成可供业务仓库安装的包：
...
4. GitHub Actions 自动创建公开 Release，并附上 `vijimlabs-ui-0.2.29.tgz`。

本包保持 `private: true` 作为 npm 误发布保护，不发布到 npm registry。业务系统切换到独立仓库前，继续使用各自已经锁定的 vendor `.tgz`。
