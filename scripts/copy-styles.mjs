...
/**
 * 本包样式入口。
 * - Mantine 样式由 provider 的 JS import 加载（避免 Next 对 CSS @import 解析失败）
 * - 此处只放：触感 / 交互态 / reduced-motion
 *   （React 19 不能把 &:hover 写进 style 对象，一律走真 CSS）
 */
const css = `/* @vijimlabs/ui — 触感与交互态（不 @import Mantine，由 provider 加载） */
...
writeFileSync(join(dist, "styles.css"), css, "utf8");
writeFileSync(join(root, "src", "styles.css"), css, "utf8");
