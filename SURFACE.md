...
| 区段 | 详情页一级业务块统一使用 `SectionHeader contained`：浅灰标题带、`1px` 外框、8px 圆角、白色内容面、常驻阴影 0；并列的同级内容共用一个外框与中间分隔线，标题必须共享父级 Grid 的同一行（可用 `subgrid`），由内容最高项自然撑开，禁止分别计算或写死像素高度。二级数据行继续只用横线和留白，禁止把每一行再做成卡片 |
